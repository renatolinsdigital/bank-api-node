 ...
 */
export const withDrawFromAccount = async (id: string | number, amount: string | number): Promise<AccountWithId> => {
  const accountId = Number(id);
 ...
 */
export const depositOnAccount = async (id: string | number, amount: string | number): Promise<AccountWithId> => {
  const accountId = Number(id);
  const operationAmount = Number(amount);
