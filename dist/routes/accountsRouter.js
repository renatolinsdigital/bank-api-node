...
import { getAllAccounts, getAccountById, createAccount, deleteAccountById, fullAccountUpdate, withDrawFromAccount, depositOnAccount, transferBetweenAccounts } from '../controllers/accountsController.js';
...
router.patch('/transfer', async (req, res, next) => {
    ...
    const { amount } = req.body;
    try {
        const { fromAccount, toAccount } = await transferBetweenAccounts(fromAccountWithId, toAccountWithId, amount);
